...
export class TradingDashboardStore implements AuditRecorder, MarketHistorySink {
  ...
  restoreOrderCards(cards: readonly DashboardOrderCard[]): void {
    ...
    for (const card of cards) {
      ...
      if (timestamp === undefined || dashboardDisplayDate(timestamp) !== this.#displayDate ||
          !dashboardUnderlyingFromSymbol(card.symbol)) continue;
  ...
  record(event: AuditEvent): void | Promise<void> {
    ...
    if (auditEventHasUnsupportedUnderlying(event)) return;
    ...
    else if (event.type === "paper_order_submission_result" || event.type === "live_order_submission_result") {
      this.#recordSubmissionResult(event);
    }
  ...
  /** Clears the in-memory read model after its durable history has been removed. */
  clearHistory(): void {
    this.#displayDate = dashboardDisplayDate(this.#now());
    this.#clearView();
    this.#orderCardPersistenceHealthy = true;
  }
  ...
  snapshot(): TradingDashboardSnapshot {
    ...
    return {
      ...
      underlyingViews: Object.fromEntries(UNDERLYING_SYMBOLS.map((underlying) =>
        [underlying, this.#buildView(generatedAt, underlying)])) as Record<UnderlyingSymbol, TradingDashboardView>,
    };
  }
  ...
  #recordDecision(event: AuditEvent): void {
    ...
    if (event.type === "live_entry_evaluation") {
      ...
      const mode = event.type === "live_order_submission_result" ? "live" : "paper";
      summary = event.data.submitted === true ? `${symbol ?? "Option"} sent to ${mode} broker` : "Order was not submitted";
...
function auditEventHasUnsupportedUnderlying(event: AuditEvent): boolean {
  const tagged = stringValue(event.data.underlying);
  if (tagged && !dashboardUnderlying(tagged)) return true;
  const candidates = [
    stringValue(event.data.symbol),
    stringValue(event.data.candidate),
    stringValue(recordValue(event.data.closestCandidate).symbol),
    stringValue(recordValue(event.data.order).symbol),
    stringValue(recordValue(event.data.position).symbol),
    stringValue(recordValue(event.data.localOrder).symbol),
  ];
  return candidates.some((symbol) => {
    if (!symbol) return false;
    const parsed = parseOccSymbol(symbol);
    return parsed !== undefined && !dashboardUnderlying(parsed.underlying);
  });
}
...
</style></head><body><main><header><div><h1>0DTE Option Day-Trade Dashboard</h1><div class="sub">Isolated ticker runtimes · shared SIP/OPRA connections · Alpaca execution · PostgreSQL history</div></div><div id="state"><span class="pulse-dot"></span><span id="stateText">Loading…</span></div></header>
<section class="scope-bar" aria-label="Underlying dashboard scope"><div class="scope-control"><label for="underlyingView">Dashboard scope</label><select id="underlyingView"><option value="ALL">All portfolio</option>${dashboardScopeOptions}</select></div><div class="scope-summary" id="scopeSummary">Showing combined ${dashboardSymbolList} activity</div></section>
...
<div class="status-card"><div class="label">Engine leadership</div><strong id="leaderState">Checking</strong><div class="status-detail" id="leaderDetail">Waiting for leader lease</div></div>
...
<div class="status-card"><div class="label">Stream queues</div><strong id="queueState">Checking</strong><div class="status-detail" id="queueDetail">Waiting for queue telemetry</div></div>
<div class="status-card"><div class="label">PostgreSQL history</div><strong id="databaseState">Checking</strong><div class="status-detail" id="databaseDetail">Verifying market event writer</div><div class="database-actions"><button class="danger-button" id="cleanupDatabase" type="button" disabled>Clean up database</button><div class="database-cleanup-status" id="databaseCleanupStatus" aria-live="polite">Available only while the system is safely idle.</div></div></div>
<div class="status-card"><div class="label">Broker execution</div><strong id="brokerState">Checking</strong><div class="status-detail" id="brokerDetail">Verifying execution connection</div></div>
<div class="status-card"><div class="label">Account trade updates</div><strong id="tradeUpdatesState">Checking</strong><div class="status-detail" id="tradeUpdatesDetail">Waiting for account stream</div></div>
...
const supportedUnderlyings=${JSON.stringify(UNDERLYING_SYMBOLS)},underlyingFromSymbol=s=>{const value=String(s||'').replaceAll(' ','').toUpperCase();if(supportedUnderlyings.includes(value))return value;const match=/^([A-Z]{1,6})\d{6}[CP]\d{8}$/.exec(value);return match&&supportedUnderlyings.includes(match[1])?match[1]:'UNKNOWN'},itemUnderlying=x=>x?.underlying||underlyingFromSymbol(x?.symbol||x?.candidate||x?.closestCandidate);
...
function setStatus(valueId,detailId,value,detail,level){$(valueId).textContent=value;$(valueId).className=level;$(detailId).textContent=detail;$(detailId).title=detail}function dashboardReadiness(h){if(h.killSwitch||!h.positionsReconciled||!h.recorderHealthy||h.leaderActive===false)return 'halted';if(!h.ready||(!h.websocketConnected&&!h.marketDataIdle)||h.marketDataBackpressure===true||h.tradeUpdateBackpressure===true||(h.brokerRequired!==false&&(!h.brokerAvailable||h.tradeUpdatesConnected===false)))return 'degraded';return 'ok'}function age(ms){return Number.isFinite(ms)?duration(ms)+' ago':'No events yet'}function storagePolicy(event,live){if(!live.persistenceEnabled)return 'Disabled';if(event.type==='stock_quote'||event.type==='option_quote')return (live.quoteSampleIntervalMs||0)+' ms baseline · active option full';return 'Full retention'}
...
try{const saved=localStorage.getItem('dashboardUnderlying');if(['ALL',...supportedUnderlyings].includes(saved))$('underlyingView').value=saved}catch{}
$('underlyingView').addEventListener('change',()=>{try{localStorage.setItem('dashboardUnderlying',$('underlyingView').value)}catch{}refresh()});
let databaseCleanupBusy=false,databaseCleanupMessage='';
function databaseCleanupBlock(h,persistenceEnabled){if(!persistenceEnabled)return 'Persistence is not enabled';if(!h.positionsReconciled)return 'Blocked until broker positions are reconciled';if(h.positionOpen)return 'Blocked while a position is open';if(h.pendingOrder)return 'Blocked while an order is pending';if((h.openOrderCount||0)>0)return 'Blocked while broker orders are open';if(h.marketClockState==='market-open'||h.marketClockState==='mixed')return 'Available only after the market closes';if(h.marketDataIdle!==true&&!/(closed|idle)/i.test(String(h.marketClockState||'')))return 'Waiting for confirmed market-closed idle state';return ''}
$('cleanupDatabase').addEventListener('click',async()=>{const phrase='${DATABASE_CLEANUP_CONFIRMATION}',entered=window.prompt('This permanently deletes all market events, audit history, orders, trades, and dashboard history. The database schema remains. Type '+phrase+' to continue.');if(entered===null)return;if(entered!==phrase){databaseCleanupMessage='Cleanup cancelled: confirmation phrase did not match.';await refresh();return}if(!window.confirm('Permanently clean up all stored trading data now? This cannot be undone.'))return;databaseCleanupBusy=true;databaseCleanupMessage='';await refresh();try{const response=await fetch('/api/database/cleanup',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({confirmation:phrase})}),result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'Database cleanup failed');databaseCleanupMessage='Database data cleared at '+new Date(result.clearedAt).toLocaleString()+'.';}catch(error){databaseCleanupMessage=String(error)}finally{databaseCleanupBusy=false;await refresh()}});
async function refresh(){
try{
const response=await fetch('/api/dashboard',{cache:'no-store'});if(!response.ok)throw new Error('Dashboard API '+response.status);
	const data=await response.json(),selected=$('underlyingView').value,view=selected==='ALL'?data:(data.underlyingViews||{})[selected]||{performance:{},liveData:{eventCounts:{},recentEvents:[]},tuning:{entries:[]},signals:[],orders:[],trades:[],decisions:[],orderCards:[],activeOrders:[]},p=view.performance||{},aggregateHealth=data.health||{},symbolHealth=(aggregateHealth.underlyingStates||{})[selected],fallbackHealth={ready:false,brokerRequired:false,websocketConnected:false,brokerAvailable:false,marketClockState:'disabled',openOrderCount:0,positionsReconciled:true,recorderHealthy:aggregateHealth.recorderHealthy!==false,killSwitch:false},h=selected==='ALL'?aggregateHealth:{...(symbolHealth||fallbackHealth),leaderActive:aggregateHealth.leaderActive},live=view.liveData||{eventCounts:{},recentEvents:[],totalEvents:0,uptimeMs:0,persistenceEnabled:false,quoteSampleIntervalMs:0,retentionDays:0},readiness=selected==='ALL'?(data.readiness||dashboardReadiness(h)):!symbolHealth?'degraded':dashboardReadiness(h);
scheduleDisplayRollover(data.nextDisplayRolloverAt);
$('scopeSummary').textContent=selected==='ALL'?'Showing combined '+supportedUnderlyings.join(' + ')+' portfolio activity':symbolHealth?'Showing isolated '+selected+' strategy, execution, P&L, tuning, feeds, and health':selected+' is not enabled in this process';
	...
	const leaderActive=aggregateHealth.leaderActive!==false;setStatus('leaderState','leaderDetail',leaderActive?'ACTIVE':'LOST',leaderActive?'PostgreSQL advisory lease owns the engine lock':'Leader lease lost · execution is halted',leaderActive?'ok':'halted');
...
const opraDetail=noSameDayOptions?'NO SAME-DAY OPTION CONTRACTS · safe idle · 0 subscriptions':opraSubscriptionIdle?'ENTRY CUTOFF · NO ACTIVE EXPOSURE · 0 subscriptions · '+count(opraQuotes)+' session WS quotes':count(opraQuotes)+' WS quotes · transport '+age(h.optionTransportAgeMs??h.lastOptionQuoteAgeMs)+' · exact symbol '+age(h.optionExactSymbolReceiveAgeMs)+' · provider '+age(h.lastOptionQuoteProviderAgeMs)+' · '+count(h.optionFreshContracts)+'/'+count(subscriptions)+' fresh contracts'+(Number.isFinite(h.optionMedianArrivalLagMs)?' · median arrival lag '+latency(h.optionMedianArrivalLagMs):'')+restFallback+opraError;
	setStatus('opraState','opraDetail',opraState,opraDetail,opraLevel);
	const queuePending=h.marketDataPendingEvents||0,queueMaximum=h.marketDataMaximumPendingEvents||0,queueLag=h.marketDataConsumerLagMs||0,queueLagMaximum=h.marketDataMaximumConsumerLagMs,queuePressure=h.marketDataBackpressure===true,queueDetail=count(queuePending)+' / '+count(queueMaximum)+' pending · lag '+latency(queueLag)+' / '+latency(queueLagMaximum)+' · '+count(h.marketDataCoalescedEvents)+' coalesced · reconnect '+count(h.reconnectAttempt);setStatus('queueState','queueDetail',queuePressure?'BACKPRESSURE':'HEALTHY',queueDetail,queuePressure?'halted':'ok');
...
const cleanupBlock=databaseCleanupBlock(aggregateHealth,live.persistenceEnabled),cleanupButton=$('cleanupDatabase');cleanupButton.disabled=databaseCleanupBusy||Boolean(cleanupBlock);cleanupButton.title=cleanupBlock||'Permanently remove all application data while retaining the schema';$('databaseCleanupStatus').textContent=databaseCleanupBusy?'Cleaning database…':databaseCleanupMessage||cleanupBlock||'Ready · permanently deletes all stored trading data.';
	...
	const tradeRequired=h.executionEnabled===true,tradeConnected=h.tradeUpdatesConnected!==false,tradePressure=h.tradeUpdateBackpressure===true,tradePending=h.tradeUpdatePendingEvents||0,tradeMaximum=h.tradeUpdateMaximumPendingEvents||0,tradeDetail=count(tradePending)+' / '+count(tradeMaximum)+' pending · lag '+latency(h.tradeUpdateConsumerLagMs||0)+' / '+latency(h.tradeUpdateMaximumConsumerLagMs)+' · reconnect '+count(h.tradeUpdateReconnectAttempt),tradeLabel=!tradeRequired?'NOT REQUIRED':tradePressure?'BACKPRESSURE':tradeConnected?'CONNECTED':'DISCONNECTED',tradeLevel=!tradeRequired||tradeConnected&&!tradePressure?'ok':'halted';setStatus('tradeUpdatesState','tradeUpdatesDetail',tradeLabel,tradeDetail,tradeLevel);
