...
export class TradingDashboardStore implements AuditRecorder, MarketHistorySink {
  ...
  snapshot(): TradingDashboardSnapshot {
    ...
    return {
      ...
      ...aggregate,
      underlyingViews: {
        SPY: this.#buildView(generatedAt, "SPY"),
        QQQ: this.#buildView(generatedAt, "QQQ"),
        GOOG: this.#buildView(generatedAt, "GOOG"),
      },
    };
  }
...
<title>SPY + QQQ + GOOG 0DTE Trading Dashboard</title><style>
...
<section class="scope-bar" aria-label="Underlying dashboard scope"><div class="scope-control"><label for="underlyingView">Dashboard scope</label><select id="underlyingView"><option value="ALL">All portfolio</option><option value="SPY">SPY only</option><option value="QQQ">QQQ only</option><option value="GOOG">GOOG only</option></select></div><div class="scope-summary" id="scopeSummary">Showing combined SPY + QQQ + GOOG activity</div></section>
...
const underlyingFromSymbol=s=>{const value=String(s||'').replaceAll(' ','').toUpperCase();if(['SPY','QQQ','GOOG'].includes(value))return value;const match=/^([A-Z]{1,6})\d{6}[CP]\d{8}$/.exec(value);return match&&['SPY','QQQ','GOOG'].includes(match[1])?match[1]:'UNKNOWN'},itemUnderlying=x=>x?.underlying||underlyingFromSymbol(x?.symbol||x?.candidate||x?.closestCandidate);
...
try{const saved=localStorage.getItem('dashboardUnderlying');if(['ALL','SPY','QQQ','GOOG'].includes(saved))$('underlyingView').value=saved}catch{}
...
$('scopeSummary').textContent=selected==='ALL'?'Showing combined SPY + QQQ + GOOG portfolio activity':symbolHealth?'Showing isolated '+selected+' strategy, execution, P&L, tuning, feeds, and health':selected+' is not enabled in this process';
