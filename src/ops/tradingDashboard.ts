...
const opraDetail=opraSubscriptionIdle?'ENTRY CUTOFF · NO ACTIVE EXPOSURE · 0 subscriptions · '+count(opraQuotes)+' session WS quotes':count(opraQuotes)+' WS quotes · receive '+age(h.lastOptionQuoteAgeMs)+' · effective provider '+age(h.lastOptionQuoteProviderAgeMs)+' · '+count(subscriptions)+' subscriptions'+restFallback+opraError;
