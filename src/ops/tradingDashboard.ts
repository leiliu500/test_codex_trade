  ...
  snapshot(): TradingDashboardSnapshot {
    ...
    return {
      ...
      underlyingViews: {
        ...
        GOOGL: this.#buildView(generatedAt, "GOOGL"),
      },
    };
  }
...
const underlyingFromSymbol=s=>{const value=String(s||'').replaceAll(' ','').toUpperCase();if(['SPY','QQQ','GOOGL'].includes(value))return value;const match=/^([A-Z]{1,6})\d{6}[CP]\d{8}$/.exec(value);return match&&['SPY','QQQ','GOOGL'].includes(match[1])?match[1]:'UNKNOWN'},itemUnderlying=x=>x?.underlying||underlyingFromSymbol(x?.symbol||x?.candidate||x?.closestCandidate);
...
try{const saved=localStorage.getItem('dashboardUnderlying');if(['ALL','SPY','QQQ','GOOGL'].includes(saved))$('underlyingView').value=saved}catch{}
