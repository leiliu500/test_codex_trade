...
import { defaultConfig, googConfig, qqqConfig } from "../config.js";
...
async function main(): Promise<void> {
  ...
  if (!path) {
    throw new Error("Usage: npm run backtest -- <events.jsonl> [conservative|midpoint-touch|queue] [calibration.json] [--symbol=SPY|QQQ|GOOG]");
  }
  ...
  const config = symbol === "QQQ" ? qqqConfig : symbol === "GOOG" ? googConfig : defaultConfig;
