...
import { defaultConfig, qqqConfig } from "../config.js";
import type { CalibrationProfile, UnderlyingSymbol } from "../types.js";
...
async function main(): Promise<void> {
  ...
  if (symbolValue !== "SPY" && symbolValue !== "QQQ") throw new Error(`Unknown replay symbol: ${symbolValue}`);
  const symbol = symbolValue as UnderlyingSymbol;
  const positional = args.filter((argument) => !argument.startsWith("--symbol="));
  const path = positional[0];
  if (!path) {
    throw new Error("Usage: npm run backtest -- <events.jsonl> [conservative|midpoint-touch|queue] [calibration.json] [--symbol=SPY|QQQ]");
  }
  ...
  const config = symbol === "QQQ" ? qqqConfig : defaultConfig;
