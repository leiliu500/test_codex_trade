...
import { defaultConfig, googlConfig, qqqConfig } from "../config.js";
...
async function main(): Promise<void> {
  ...
  if (!path) {
    throw new Error("Usage: npm run backtest -- <events.jsonl> [conservative|midpoint-touch|queue] [calibration.json] [--symbol=SPY|QQQ|GOOGL]");
  }
  ...
  const config = symbol === "QQQ" ? qqqConfig : symbol === "GOOGL" ? googlConfig : defaultConfig;
