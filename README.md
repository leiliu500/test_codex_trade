...
The option-only/day-only constraints are enforced at the contract-universe, selector, order-state, filled-position, and broker REST boundaries. New entries stop at the configured 0DTE cutoff (14:30 ET by default), regardless of the entry-quality research mode. The default `signals.entryQualityMode` is `SHADOW`: the 13:00 bullish-impulse cutoff, causal 5–15 second confirmation, six-entry daily cap, and early-scratch proposal are calculated and audited but cannot delay an entry, reject a paper order, or close a paper position. The live audit evaluates bullish-impulse, all-impulse, and all-entry confirmation scopes side by side; bullish impulses are the recommended primary shadow profile. Daily fill counts are restored across restarts for both active and shadow risk decisions. All broker orders use `time_in_force=day`, and open positions receive a mandatory marketable-limit exit at 15:50 ET—before same-day expiration. A stock symbol, non-SPY option, or later-dated SPY option is rejected before submission.
