...
The option-only/day-only constraints are enforced at the contract-universe, selector, order-state, filled-position, and broker REST boundaries. Every new entry order is hard-limited to exactly one option contract; the standard OCC 100-share contract multiplier remains unchanged for premium, risk, and P&L accounting. New entries stop at the configured 0DTE cutoff (14:30 ET by default), regardless of the entry-quality research mode. Beginning at 12:00 ET, the active paper path applies a time-adjusted late-entry guard: every entry must project at least 2.0 bps, confirm at least 0.1 bps of directional follow-through during a causal 5–15 second window, retain at least 1.0 bps of modeled edge after option costs, and use an option spread no wider than 0.8%. Morning entries retain their existing behavior. The separate `signals.entryQualityMode` remains `SHADOW`: the 13:00 bullish-impulse cutoff, general confirmation profiles, six-entry daily cap, and early-scratch proposal are calculated and audited but cannot independently delay an entry, reject a paper order, or close a paper position. The live audit evaluates bullish-impulse, all-impulse, and all-entry confirmation scopes side by side. Daily fill counts are restored across restarts for both active and shadow risk decisions. All broker orders use `time_in_force=day`, and open positions receive a mandatory marketable-limit exit at 15:50 ET—before same-day expiration. A stock symbol, non-SPY option, or later-dated SPY option is rejected before submission.
...
Configuration lives in [`config/default.json`](config/default.json). Calibration profiles must contain only sessions strictly before the replayed session. The historical signal test reports a `guardComparison` block that evaluates immediate entry, bullish-impulse confirmation, all-impulse confirmation, and all-entry confirmation on the same downloaded tape; its forward returns are research labels and never enter the causal signal decision. Active late-entry state and thresholds are recorded under `live_entry_evaluation.data.lateEntryGuard`, `live_signal_selection.data.lateEntryGuard`, and `paper_order_submission_result.data.lateEntryGuard`; signal and option failures use explicit `LATE_ENTRY_*` reasons. `live_entry_evaluation.data.lateEntryBaseline` is a non-executable evaluator with the late guard disabled, preserving the baseline candidates needed to measure the active guard. Other shadow evidence remains available in `live_entry_evaluation.data.shadowEvaluations`, `risk_decision.data.shadowRisk`, `shadow_exit_decision`, and `daily_risk_state_recovery.data.shadowEntryCapReached`.
