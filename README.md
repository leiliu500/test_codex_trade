...
The option-only/day-only constraints are enforced at the contract-universe, selector, order-state, filled-position, and broker REST boundaries. Every new entry order is hard-limited to exactly one option contract; the standard OCC 100-share contract multiplier remains unchanged for premium, risk, and P&L accounting. New entries stop at the configured 0DTE cutoff (14:30 ET by default), regardless of the entry-quality research mode. From 10:15 through 11:59:59 ET, the active paper path applies a static morning-entry guard: every entry must project at least 1.7 bps, retain at least 1.0 bps of modeled edge after option costs, and use an option spread no wider than 0.75%; it adds no follow-through delay. Beginning at 12:00 ET, the existing late-entry guard requires at least 2.0 projected bps, at least 1.0 bps of modeled post-cost edge, and an option spread no wider than 0.8%. Most late setups also require at least 0.1 bps of directional follow-through during a causal 5–15 second window. Qualifying bearish grinds enter without an extra delay because their medium/slow trend gates already encode persistence; unclassified bearish impulses do the same only before 13:00 ET. Late bearish `STRONG_DOWN` impulses retain the stricter profile observed to distinguish the July 27 continuation attempts: at least 1.5 bps at the causal five-second check. Other late setups retain the full window, including the July 24 winning bullish impulse that confirmed at 15 seconds. The guard's separate daily-entry ceiling remains at the high 1,000-entry safety limit. The separate `signals.entryQualityMode` remains `SHADOW`: the 13:00 bullish-impulse cutoff, general confirmation profiles, global six-entry daily cap, and early-scratch proposal are calculated and audited but cannot independently delay a morning entry, reject a morning paper order, or close a paper position. The live audit evaluates bullish-impulse, all-impulse, and all-entry confirmation scopes side by side. Daily fill counts are restored across restarts for both active and shadow risk decisions. All broker orders use `time_in_force=day`, and open positions receive a mandatory marketable-limit exit at 15:50 ET—before same-day expiration. A stock symbol, non-SPY option, or later-dated SPY option is rejected before submission.
...
- Account/risk/premium/buying-power sizing caps, restart-continuous daily ET limits, conservative bid-based executable P&L, direct/recovered winner states, first-passage recovery probability, nondecreasing protected-profit floors, reversal evidence, recovery deadlines, and priority exits.
- Live and replay option-continuation LCBs using delta, gamma, vega, theta, IV change, physical SPY slope, executable spread cost, and observed P&L uncertainty; stale snapshots are excluded and negative estimates require persistence.
