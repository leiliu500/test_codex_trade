...
Every marketable exit attempt has a configured replacement budget and cancellation deadline. A canceled attempt preserves one logical exit intent and reprices only the remaining broker-confirmed position. The logical exit has its own five-attempt and two-minute safety bounds; exceeding either bound, or waiting too long for a cancel acknowledgement, halts explicitly instead of polling forever or risking a duplicate sell.
