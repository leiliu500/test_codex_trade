...
The option-only/day-only constraints are enforced at the contract-universe, selector, order-state, filled-position, and broker REST boundaries. Every new entry order is hard-limited to exactly one option contract; the standard OCC 100-share contract multiplier remains unchanged for premium, risk, and P&L accounting. New entries stop at the configured 0DTE cutoff (14:30 ET by default), regardless of the entry-quality research mode. From 10:15 through 11:59:59 ET, the active paper path applies a static morning-entry guard: every entry must project at least 1.7 bps, retain at least 1.0 bps of modeled edge after option costs, and use an option spread no wider than 0.75%; it adds no follow-through delay. Beginning at 12:00 ET, the existing late-entry guard requires at least 2.0 projected bps, at least 0.1 bps of directional follow-through during a causal 5–15 second window, at least 1.0 bps of modeled post-cost edge, and an option spread no wider than 0.8%. The separate `signals.entryQualityMode` remains `SHADOW`: the 13:00 bullish-impulse cutoff, general confirmation profiles, six-entry daily cap, and early-scratch proposal are calculated and audited but cannot independently delay an entry, reject a paper order, or close a paper position. The live audit evaluates bullish-impulse, all-impulse, and all-entry confirmation scopes side by side. Daily fill counts are restored across restarts for both active and shadow risk decisions. All broker orders use `time_in_force=day`, and open positions receive a mandatory marketable-limit exit at 15:50 ET—before same-day expiration. A stock symbol, non-SPY option, or later-dated SPY option is rejected before submission.
