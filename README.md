# Multi-Underlying 0DTE Options Intraday Trend Engine

A deterministic TypeScript implementation of the attached **SPY Options Intraday Trend System** baseline with isolated SPY, QQQ, GOOGL, IWM, GLD, XLF, XLE, TSLA, NVDA, SNDK, SPCX, DIA, and XLK trading runtimes. Paper execution is the default. Underlying top-of-book and trades are read-only signal inputs; the engine never submits orders for shares. Every executable instrument must be an OCC-formatted option for its runtime's underlying and expire on the current New York market date (0DTE).
...
Compose reads broker credentials from the local `.env` file at runtime and never copies that file into the image. It forces `TRADING_MODE=paper` and enables broker-backed paper orders. `TRADING_SYMBOLS` controls the enabled subset; Compose enables `SPY,QQQ,TSLA` by default, while the remaining supported symbols stay disabled unless explicitly requested and a standalone process without the variable retains the conservative SPY-only default. It also starts PostgreSQL on the private Compose network; the database port is not exposed publicly. `/live` reports process liveness. `/ready` returns 200 only after PostgreSQL, the leader lease, required WebSocket feeds, bounded internal queues, paper account/options approval, account-update connectivity, and every enabled runtime's scoped broker reconciliation are healthy. The Alpaca account must have real-time SIP and OPRA entitlement. Set `ENABLE_LIVE_ORDERS=false` to receive SIP without submitting paper orders; set `MARKET_DATA_ENABLED=false` as well for paper-idle mode. Readiness remains degraded in either reduced mode.
...
## Production multi-ticker controls

All enabled tickers are multiplexed over one physical equity SIP stream and one physical OPRA stream. The account-level order stream is a third connection to Alpaca's separate trading WebSocket endpoint, not another market-data connection. Alpaca documents that most market-data subscriptions allow only one connection to a given endpoint, so opening a WebSocket per ticker is not supported. It also documents a 1,000-option-quote WebSocket subscription ceiling for Algo Trader Plus. The engine therefore defaults `OPRA_MAX_SUBSCRIPTIONS` to 900, leaving headroom for subscription transitions; lower it to the actual limit of the account rather than assuming the plan. See Alpaca's [market-data plan limits](https://docs.alpaca.markets/us/docs/about-market-data-api), [market-data connection errors](https://docs.alpaca.markets/us/v1.1/docs/streaming-market-data), and [account trade-update stream](https://docs.alpaca.markets/us/docs/websocket-streaming).

The shared hubs own reconnect and subscription state. A physical disconnect is generation-fenced, reconnected once with exponential backoff, and then restores the union of all active ticker subscriptions. Per-ticker consumers have independent dispatch queues, so a slow QQQ consumer cannot block SPY. New entries fail closed when a queue exceeds `MARKET_DATA_MAX_PENDING_PER_SYMBOL`, internal consumer lag exceeds `MARKET_DATA_MAX_INTERNAL_LAG_MS`, the account update stream disconnects, or its per-underlying queue exceeds `TRADE_UPDATE_MAX_PENDING_PER_SYMBOL`. OPRA execution quotes are coalesced to the newest provider timestamp per contract while raw history remains bounded and ordered.

Order fills, partial fills, cancellations, rejections, and replacements arrive through one account `trade_updates` stream and are routed by OCC underlying. Every initial connection and reconnect performs REST reconciliation before the runtimes become executable. An unmatched account order update or conflicting broker state halts execution rather than guessing ownership. PostgreSQL advisory-lock leadership permits only one engine instance per `LEADER_LOCK_KEY`; losing that database session immediately removes readiness and shuts down all streams and order submission.

The relevant capacity controls are shown in [`.env.example`](.env.example):

```text
TRADING_SYMBOLS=SPY,QQQ,TSLA
OPRA_MAX_SUBSCRIPTIONS=900
MARKET_DATA_MAX_PENDING_PER_SYMBOL=50000
MARKET_DATA_MAX_INTERNAL_LAG_MS=500
TRADE_UPDATE_MAX_PENDING_PER_SYMBOL=10000
LEADER_ELECTION_ENABLED=true
LEADER_LOCK_KEY=alpaca-options-engine
LEADER_HEARTBEAT_MS=5000
```

Live promotion additionally requires all of the following. The checked-in Compose file intentionally forces paper mode; use a separately reviewed live deployment manifest rather than trying to toggle it through that file.
...
Set `PARITY_POST_FILL_CONTEXT_MS` to a non-negative millisecond window when a counterfactual exit configuration needs more retained tape after the observed live fill. The default remains 5,000 ms for strict live parity checks.
...
`src/alpaca/` includes authenticated Alpaca stock JSON streaming, option MsgPack streaming, paper/live account-update streaming and REST selection, contract/snapshot pagination, whole-contract option orders, and symbol-scoped broker reconciliation without embedding credentials or silently enabling real-money orders. The runtime routes every enabled ticker's SIP events through isolated feature/signal engines, selects only each runtime's same-day options using routed OPRA quotes, and sends eligible entries and protective exits to the explicitly selected Alpaca paper or live endpoint. A narrowly defined late bullish reentry profile admits exceptionally clean, low-noise grinds after eight minutes without removing ordinary causal confirmation or the ten-minute cooldown from other setups. Unknown broker state halts the affected execution runtime. Live mode is rejected unless execution, data, credentials, dedicated confirmation, PostgreSQL leadership, and a database URL are all configured.
