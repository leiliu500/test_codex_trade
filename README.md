# SPY + QQQ Options Intraday Trend Engine

A deterministic TypeScript implementation of the attached **SPY Options Intraday Trend System** baseline with an isolated QQQ paper-trading runtime. SPY and QQQ top-of-book and trades are read-only signal inputs; the engine never submits orders for ETF shares. Every executable instrument must be an OCC-formatted option for its runtime's underlying and expire on the current New York market date (0DTE).
...
The browser polls `/api/dashboard` once per second. Its dashboard-scope selector provides exact `All portfolio`, `SPY only`, and `QQQ only` views. Each per-symbol view has independent signal funnels, orders, trades, realized/open P&L, tuning statistics, missed-entry denominators, market-event counts, and runtime health; the selection is retained in the browser. Dashboard state is reconstructed from PostgreSQL after a service restart. The SIP and OPRA cards use separate live receiver counters; the strategy card labels the SIP events restored once at startup. The live OPRA boundary coalesces each received frame and queued updates to the newest quote per contract so repeated high-rate updates cannot create a stale backlog; replay-oriented raw retention remains sampled by the configured history policy. Provider-time age beyond the configured quote-freshness limit degrades readiness and blocks entries without reconnect churn, allowing the bounded consumer to catch up. Ten seconds of receive silence marks the option feed stalled and triggers a reconnect; after a connection opens, readiness remains fail-closed until a new provider-timestamped quote arrives.

When WebSocket provider timestamps exceed the same freshness limit, the runtime requests the latest quotes for only its currently subscribed contracts from Alpaca's latest-options endpoint with `feed=opra` explicitly set. A REST quote must pass the unchanged provider-time, market-date, symbol, and price validation before it can update selection or manage an open position. Fresh REST data keeps the effective option price current and schedules a rate-limited WebSocket reconnect only when its provider timestamp proves it is ahead of the stream. If both paths are stale or REST fails, readiness stays degraded and entries remain blocked; the fallback never substitutes indicative data or relaxes the two-second freshness guard. The dashboard exposes REST request, fresh-quote, age, in-flight, and error telemetry separately from WebSocket receive telemetry.

PostgreSQL persists two indexed histories in the named `spy-options-postgres` volume:

- `market_events`: raw SPY/QQQ SIP quotes/trades, subscribed OPRA option quotes, option contracts/snapshots, and generated feature snapshots. High-rate records are inserted in batches so quote handling is not blocked by one SQL round trip per event.
