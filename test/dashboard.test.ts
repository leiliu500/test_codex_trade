...
test("dashboard exposes liveness and feed tabs before any entries, orders, or history", () => {
  ...
  assert.match(html, /GLD only/);
  assert.match(html, /XLF only/);
  assert.match(html, /XLE only/);
  assert.match(html, /TSLA only/);
  assert.match(html, /NVDA only/);
  assert.match(html, /SNDK only/);
  assert.match(html, /SPCX only/);
  assert.match(html, /DIA only/);
  assert.match(html, /XLK only/);
  ...
  assert.ok(snapshot.underlyingViews.GLD);
  assert.ok(snapshot.underlyingViews.XLE);
  assert.ok(snapshot.underlyingViews.XLK);
